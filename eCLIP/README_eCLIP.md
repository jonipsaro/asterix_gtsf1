...
4. Calculation of enrichment scores
...
2. If needed, join paired-ends using FLASH.  Example:
...
3. Remove adapters and extraneous sequences by comparing the FLASH input and output. 
    The script will prompt for the input and output file names.  Full paths are recommended. Output can be directed to the 3_adapter_trimmed folder.
    ```python 1_read_cleanup/flash_trim.py```

    Count the total number of adapter-trimmed reads and the number of adapter dimers.  This will be needed for library normalization in the final enrichment step.
	```python 1_read_cleanup/count_adapters_only.py```

4. Unique Molecular Identifier (UMI) collapse
    Remove PCR duplicates by comparing UMIs within each file.  The script will prompt to specify the file names for the inputs. Output can be directed to the 4_remove_pcr_duplicates folder.
    **Note: The library construction is slightly different for the input samples versus the CLIP samples.  Make sure to run the correct version of umi_collapse.py.*
	```python 1_read_cleanup/umi_collapse.py```
	```python 1_read_cleanup/umi_collapse_inputs.py```

5. Reverse complement
    The script will prompt to specify the file names for the inputs. Output can be directed to the 5_reverse_complement folder.
    **Note: eCLIP libraries are stranded with the reverse complement corresponding to sense strand of the input.*
...
3. Clean up the mapping to remove "multi-aligners" that are not true "multi-mappers."
    ```python 2_mapping_cleanup/unique_bed_tidy.py```

## Part 3a - Annotation
**Note:  Annotation is performed with bedtools.  The -f 1 option requires complete intersection "a" within "b"; -s enforces strandedness.*
**Note:  The path to the annotation .bed file must be specified for file "b".*
1.  Annotate
...
2. Format the no_annotation file so that is consistent with the annotated file list
	```python 3_annotation_and_filtering/format_no_annotation.py```

3.  Create a composite file with all reads labeled with either their annotation or "no annotation"
...
Most usually, the filtering order is based on transcript abundance, but the order can be modified with the scripts.
0. Set up directories.
    **Note: Subdirectories can be useful if you want to try multiple filtering orders (see below and the filter_by_class.py script).*
	```mkdir 10_filtered```

1. Determine ALL of the annotation classes in the main annotation .bed file
...
3. Perform filtering. This step is somewhat slow, so it may be best to run it on a server. Output will go to the 10_filtered folder
	```python 3_annotation_and_filtering/filter_by_class.py```

## Part 4 - Calculate Enrichment
### Option 1 - Simple Enrichment
This option will normalize multimappers within each class, calculate annotation pileups, subtract background, and calculate fold-changes.
0. Set up directories
	```mkdir 11a_normalized 12a_pileup 13a_enrichment```

1. Normalize multi-mappers within each class (1/n-normalization).
...
3. Make a composite file for each of the libraries:
    ```cat S1/*.txt > S1/S1-all_normalized.txt```

4. Generate the summary:
    ```python 4a_simple_enrichment/reads_per_annotation.py```

5. Calculate the annotation pileups for each library.
	```python 4a_simple_enrichment/pileup_by_annotation.py```

6. Move the output files to the 12a_pileup directory.

7. Combine pileups into a single file. This is useful for Option 2 (below), but is convenient to do here.
...
8. Calculate the enrichment for each library
	...
	- Low read-count thresholds can also be adjusted in the script.  A value which gives 1 read per input library and 10 reads per pull-down library seem reasonable.

	```python 4a_simple_enrichment/enrich_calc4.py```

4. File clean-up
    - Move the enrichment file outputs to the 13a_enrichment directory.
    - Make the final enrichment file consistent and sorted by enrichment score
        ```sort -nk8 -r S123456-enrich_per_annotation.txt > S123456-enrich_per_annotation_sorted.txt; \```
...
1. Build the alternative gene model (i.e. "logical genes")
**Note: Make sure all classes are accounted for.  This should be the same list as used in filter_by_class.py (above).  Class order does not matter for this script.*
...
A prep script was written that takes *-all-logical_genes.txt files for each sample and builds a composite file: [logical_gene, S1_value, S2_value, ...]
...
The output file will have the following tab-delimited columns:
    ...
    **Note: p-values will be the less significant value of [ip versus input; ip versus background])*

    ```python 4b_deseq2/deseq2_polish.py```

6. File cleanup - Add no_annotation class
...
Note: p-value thresholds can be set in the logical_to_real.py script.
